...
The packager doesn't impose four packages — it produces however many entries are in `target.yaml`'s `packaging.deb.binaries[]`. TensorFlow Lite, having no EP plugins and no `-dev`, ships two runtime packages: `libtensorflowlite-c` (the flat, unversioned C API `.so`) and `libtensorflow-lite2.19` (the versioned C++ runtime `.so`, soversion in the name like `libonnxruntime1.22`).
