...
- **Verify the Jetson CUDA EP `depends` package names**: the `linux-aarch64-jp62-cuda126` EP package depends on `libcudart12`, `libcublas12`, `libcudnn9`, etc. — the Debian/Ubuntu CUDA-repo spellings. JetPack/L4T may ship these via `nvidia-jetpack`/`cuda-libraries-*` metapackages under different installable names. Confirm against a real JetPack 6.2 apt cache (`apt-cache show libcudart12`) before publishing the CUDA EP, or the package won't resolve on the hardware it targets. (The static ABI check validates SONAME *linkage*, which is a separate axis from apt *depends*.)
