...
- `tflite` ships **Linux x86_64 + aarch64 only** (CPU-only, CMake build of the TFLite C API — *not* bazel). macOS/Windows are intentionally absent (served by ONNX Runtime). Its library is a **flat unversioned `.so`**. Like onnxruntime it ships **no `-dev` package** (consumption is dlopen-only), so its Debian split is a single `libtensorflowlite-c`; headers still go in the tarball as a **nested tree** under `include/tensorflow/lite/c/` via the recipe's `headers: [{src, dest}]` form.
