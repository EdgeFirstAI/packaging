...
- `onnxruntime` packages are **layered so CUDA is optional**. The arch-generic base lib (`libonnxruntime1.22`) and `-providers-shared` are built **once per arch** by CPU targets (`targets/linux-amd64` → key `linux-x86_64`, `targets/linux-arm64` → key `linux-aarch64`) and carry no CUDA linkage. The Jetson target (`linux-arm64-jp62-cuda126`) packages **only** the CUDA EP `.deb` (its tarball still bundles everything). Do not re-add base/providers-shared to the Jetson target's `packaging.deb.binaries` — that recreates a `libonnxruntime1.22_arm64.deb` collision with the CPU target. **onnxruntime ships no `-dev` package** (consumption is Rust/dlopen, no headers; also avoids the `libonnxruntime-dev` name collision with Ubuntu 24.04+/Debian) — headers still go in the tarball; do not re-add a `-dev` deb. The Jetson CUDA EP `depends` use JetPack package names (`cuda-cudart-12-6`, `libcublas-12-6`, `libcufft-12-6`, `libcudnn9-cuda-12`), NOT the x86 CUDA-repo names (`libcudart12`…) which don't exist on JetPack. x86_64+CUDA is planned follow-up (ARCHITECTURE.md open issues). `package-deb.sh` substitutes `${binary:Version}`/`${source:Version}` itself (it calls `dpkg-deb` directly, not `dpkg-gencontrol`).
- `tflite` ships **Linux x86_64 + aarch64 only** (CPU-only, CMake build — *not* bazel; XNNPACK ON). macOS/Windows are intentionally absent (served by ONNX Runtime). It builds **two** libraries in one pass (per-target `build.sh` runs both CMake projects and stages both into `_build`): the **C API** `libtensorflowlite_c.so` (flat, unversioned) and the **C++ runtime** `libtensorflow-lite.so.2.19.0` (versioned SONAME, built from `tensorflow/lite/` via `patches/2.19.0/0001-cmake-shared-cpp-lib.patch` which adds `TFLITE_BUILD_SHARED_LIB`). The `build_layout.libraries.main` glob is `libtensorflow*.so*` so `cp -P` collects both chains. Like onnxruntime it ships **no `-dev` package** (consumption is dlopen-only); its Debian split is two runtime packages, `libtensorflowlite-c` and `libtensorflow-lite2.19` (soversion in the name). Only the C API headers go in the tarball as a **nested tree** under `include/tensorflow/lite/c/` via the recipe's `headers: [{src, dest}]` form; no C++ headers. The C++ SONAME is versioned at build time by the patch (`SOVERSION`), so the recipe does **not** set `version_soname`. Both targets use **`-xlarge` runners** because building both libraries (especially the C++ shared-link) is memory-hungry.
- onnxruntime ships a **version-specific soname** `libonnxruntime.so.1.22` (not the bare-major `.so.1`), opted in via the recipe's `build_layout.libraries.version_soname: true`. `run-build.sh` applies it post-test/pre-package via `patchelf --set-soname` (so build hosts need `patchelf`); it drops the `.so`/`.so.1` symlinks. Don't reintroduce a bare-major soname — it must not shadow a newer system onnxruntime. The amd64 build also forces a non-executable stack (`LDFLAGS=-Wl,-z,noexecstack` in `build.sh`); don't drop it (Ubuntu 26.04's loader rejects an exec-stack `.so`).
